...
- **Frontend**: Vercel Git integration with **Root Directory = `apps/web`**. Vercel reads `apps/web/vercel.json`. Install runs from the monorepo root (`cd ../.. && bun install --frozen-lockfile`) so workspace deps and `bun.lock` stay in sync. Bun is pinned to **1.3.7** (`.bun-version`) so CI, local installs, and the checked-in lockfile use the same Bun release. Production builds use Turbopack (`next build --turbopack` in `apps/web/package.json`).
